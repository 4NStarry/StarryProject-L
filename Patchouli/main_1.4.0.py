...
def select():
	...
	try:
		...
		try:
			...
			pass
def pop():
	QMessageBox.information(widget,'StarryProject-L 1.4.0 "Save the mind"',"建筑损伤图像贴标程序\n版本　1.4.0　2024/10/23\nby　4N☆E　<Starry4N@stu.scu.edu.cn>\n2024　StarryProject™\n内部软件　请勿外传\n\nV1.4新增特性：\n1.输入框更改为下拉列表，进入模块将立即加载目录中的首个JPG文件并显示；\n2.「混合式」模块已经替代「整体」模块，在不选择构件时的输出与曾经的「整体」模块完全一致；\n3.当前目录的显示将即时更新。\n\n\n——哇啊，好多书啊\n　　等一下全都爽快地借走\n——不要拿")
app=QApplication.instance()
if app is None:
	app=QApplication(sys.argv)
widget=QWidget()
widget.setWindowTitle('StarryProject-L 1.4.0')
