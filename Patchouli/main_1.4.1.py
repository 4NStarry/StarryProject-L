...
def mix():
	...
	add2.button1.clicked.connect(remove)
	...
	add2.show()
	add2.combo.setFocus()
	add2.setFocusPolicy(Qt.StrongFocus)
def partial():
	widget.close()
	global add1
	add1=QWidget()
	...
	add1.show()
	add1.combo.setFocus()
	add1.setFocusPolicy(Qt.StrongFocus)
def display():
	global name
	try:
...
def add_mix():
	if add2.group0.checkedButton().text():
		...
		if num==0:
			...
			content='{"image_id": "'+name+'",'+'"caption": "In this picture, there is a '+add2.group0.checkedButton().text()+' building. Under earthquake, the building has experienced '+add2.group00.checkedButton().text()+' collapse, the state of the building was '+add2.group3.checkedButton().text()+' damage."},'
...
def select():
	...
	try:
		...
		try:
			...
			pass
def pop():
	QMessageBox.information(widget,'StarryProject-L 1.4.1 "Agni Shine"',"建筑损伤图像贴标程序\n版本　1.4.1　2024/10/23\nby　4N☆E　<Starry4N@stu.scu.edu.cn>\n2024　StarryProject™\n内部软件　请勿外传\n\nV1.4新增特性：\n⒈输入框更改为下拉列表，进入模块将立即加载目录中的首个JPG文件并显示；\n⒉「混合式」模块已经替代「整体」模块，在不选择构件时的输出与曾经的「整体」模块完全一致；\n⒊当前目录的显示将即时更新。\nV1.4.1：⒈修复「局部」模块布局显示不正常，根本无法进行贴标的恶性BUG；⒉优化图像显示布局;⒊各模块通过方向键「上」「下」可切换图像。\n\n\n——就要拿\n——让我看看，如何把眼前的\n　　黑色给消极地处理掉…")
app=QApplication.instance()
if app is None:
	app=QApplication(sys.argv)
widget=QWidget()
widget.setWindowTitle('StarryProject-L 1.4.1')
