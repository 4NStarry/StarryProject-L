...
def entire():
    ...
    add0.radio_button1=QRadioButton('masonry')
...
def partial():
    ...
    add1.layout.addWidget(add1.label6,30,0)
    ...
    add1.layout.addWidget(add1.button0,31,1)
    add1.layout.addWidget(add1.button1,32,1)
    add1.layout.addWidget(add1.button3,33,1)
    ...
    add1.layout.addWidget(add1.list_widget,31,0,3,1)
    add1.layout.addWidget(add1.input_text,33,0)
    ...
    add1.check4.stateChanged.connect(ban)
    add1.check5.stateChanged.connect(ban)
...
def display1():
    ...
    add1.layout.addWidget(add1.label5,0,0,31,1)
def close0():
    add0.close()
    widget.show()
def close1():
...
def rgroup2():
    ...
    if add1.radio_button09.isChecked()==False and add1.radio_button10.isChecked()==False:
        ...
        #     add1.check40.setDisabled(True)
def ban():
    if add1.check4.isChecked()==True or add1.check5.isChecked()==True:
        add1.check2.setDisabled(True)
        add1.check2.setChecked(False)
        add1.check3.setDisabled(True)
        add1.check3.setChecked(False)
...
def pop():
    msgbox=QMessageBox.information(widget,'StarryProject-L 1.1.1 "Moonlight Ray"',"地震震后建筑照片贴标程序\n版本　１.１.１　２０２４／７／２０\nｂｙ　４Ｎ☆Ｅ　＜Ｓｔａｒｒｙ４Ｎ＠ｓｔｕ．ｓｃｕ．ｅｄｕ．ｃｎ＞\n２０２４　ＳｔａｒｒｙＰｒｏｊｅｃｔ™\n内部软件　请勿外传\n\nＶ１.１新增特性：\n１.ＵＩ改进；\n２.在ＲＣ结构下的墙将被写入为非结构构件填充墙；\n３.「坍塌」多选框将在大多数情况下可用；\n４.在“concrete spalling”或“buckling of reinforcement bars”被选中时，“cracks”和“facades spalling”多选框将不可用。\nＶ１.１.１：修复了「整体结构」板块生成错误语句的问题。\n\n\n——这种心情，怎么说的来着……\n要是那家伙的话肯定会说‘好舒服啊’。\n我可是不喜欢夜晚，怎么说都只有奇怪的家伙而已。")
    if msgbox==QMessageBox.Ok:
        QMessageBox.about(widget,'已知问题',"１.浏览文件夹按钮选择新文件夹后需要返回主菜单再进入才能更新显示的路径；\n２.无法使所有单选按钮的显示文本为中文的同时输出内容为英文，以现有代码的逻辑无法做到。")
app=QApplication([])
widget=QWidget()
widget.setWindowTitle('StarryProject-L 1.1.1')
